...
<?php $this->section('main'); ?>
    <x-page-head>
        <a href="<?php echo $backUrl ?>" class="back">&larr; <?= lang('crud.materi') ?></a>
        <h4><?php echo isset($data) ? '<i class="fa fa-pencil"></i>' : '<i class="fa fa-plus"></i>' ?>  <?= lang('crud.materi') ?></h4>
    </x-page-head>

    <?php if (isset($data) && null !== $data->deleted_at) { ?>
        <div class="alert danger">
            This <?= lang('crud.materi') ?> was deleted on <?php echo $data->deleted_at->humanize(); ?>.
            <a href="#">Restore <?= lang('crud.materi') ?>?</a>
        </div>
    <?php } ?>
    ...
    <x-admin-box>
        ...
        <form action="<?php echo $actionUrl; ?>" method="post" enctype="multipart/form-data">
            ...
            <fieldset>
                                <div class="row mb-3">
                    <?= form_label(lang('crud.bab_id'),'',['for' => 'bab_id', 'class' => 'col-form-label col-sm-2']) ?>
                    <div class="col-sm-10">
                        <?= form_dropdown('bab_id',$babItems ,old('bab_id', $data->bab_id ?? ''), "class='form-control select2' required placeholder='".lang('crud.bab_id')."' ") ?>
                        <?php if (has_error('bab_id')) { ?>
                        <p class="text-danger"><?php echo error('bab_id'); ?></p>
                        <?php } ?>
                    </div>
                </div>
                <div class="row mb-3">
            ...
            <div class="text-end py-3">
                <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-save"></i> <?= lang('crud.materi') ?></button>
            </div>
